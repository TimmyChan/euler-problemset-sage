...
```
    ...
    4.25 s ± 75.8 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
    5.34 µs ± 32.1 ns per loop (mean ± std. dev. of 7 runs, 100,000 loops each)
...
```
    ...
    2.57 ms ± 115 µs per loop (mean ± std. dev. of 7 runs, 100 loops each)
    245 µs ± 3.69 µs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)
...
```
    ...
    5.53 µs ± 101 ns per loop (mean ± std. dev. of 7 runs, 100,000 loops each)
    71 * 839 * 1471 * 6857
...
```
    ...
    1.85 µs ± 20 ns per loop (mean ± std. dev. of 7 runs, 1,000,000 loops each)
...
\therefore \left( \sum_{k=1}^k n  \right)^2 - \sum_{n=1}^k n^2 &= \left( \frac { n(n+1)} 2 \right)^2 - \frac{n(n+1)(2n+1)}{6} \\
...
```
    ...
    3.34 ms ± 36.3 µs per loop (mean ± std. dev. of 7 runs, 100 loops each)
    889 ns ± 6.39 ns per loop (mean ± std. dev. of 7 runs, 1,000,000 loops each)
...
```
    ...
    1.17 µs ± 13.9 ns per loop (mean ± std. dev. of 7 runs, 1,000,000 loops each)
...
__Remarks:__
Let $k$ be some even integer (our perimeter)
\begin{align}
a^2 + b^2 &= (k - (a+b))^2 \\
a^2 + b^2 &= k^2 - 2k(a+b) + (a+b)^2 \\
a^2 + b^2 &= k^2 - 2ka -2kb + a^2 +2ab + b^2 \\
\therefore 0 &= ab - ka - kb + \frac{k^2}{2} \\
\end{align}
...
integer_solns = solve([a*b - 1000*a - 1000*b + 500000 == 0],[a,b])
solns = [(entry[0],entry[1]) for entry in integer_solns if (entry[0] >0 and entry[1] >0 and entry[0]<entry[1] and entry[0]+entry[1]<perimeter)]
print(solns)
a = solns[0][0]
b = solns[0][1]
c = perimeter - a - b
...
k = 0
ans = 0
while prime.unrank(k) < upperbound:
    new = prime.unrank(k)    
    # print("n=%d,\t\t%d + %d = %d"% (k, ans, new, ans + new))
    ans += new
    k += 1
...
__Remark:__
Note that this is applying the standard sigma (factor counting) function on triangle numbers. Triangle numbers can be defined explicitly where $T_n = \frac{n(n+1)}2$
...
def triangle(n):
    return n * (n+1) / 2
k = 1
while sigma(triangle(k), 0) < 500:
    k += 1
print("%d has %d factors: %s" % (triangle(k), sigma(triangle(k),0), divisors(triangle(k-1))))
```

    76576500 has 576 factors: [1, 3, 5, 9, 11, 15, 23, 25, 33, 45, 55, 69, 75, 99, 115, 125, 165, 207, 225, 253, 269, 275, 345, 375, 495, 575, 759, 807, 825, 1035, 1125, 1265, 1345, 1375, 1725, 2277, 2421, 2475, 2875, 2959, 3795, 4035, 4125, 5175, 6187, 6325, 6725, 8625, 8877, 11385, 12105, 12375, 14795, 18561, 18975, 20175, 25875, 26631, 30935, 31625, 33625, 44385, 55683, 56925, 60525, 68057, 73975, 92805, 94875, 100875, 133155, 154675, 204171, 221925, 278415, 284625, 302625, 340285, 369875, 464025, 612513, 665775, 773375, 1020855, 1109625, 1392075, 1701425, 2320125, 3062565, 3328875, 5104275, 6960375, 8507125, 15312825, 25521375, 76564125]
...
$$
a_{n+1}  = \begin{cases}\frac n 2 & n \text{ even} \\
3n +1 & n \text{ odd} \end{cases}
$$
...
$$P(BB) = \left(\frac{15}{21}\right)\left(\frac{14}{20}\right) = \frac 1 2.$$
...
```python
total = Integer(10)
blue = Integer(ceil(sqrt(2)/2*total))
while total < 1000000:
    while (blue/total)*((blue-1)/(total-1)) != 1/2:
        total +=1
        blue = Integer(ceil(sqrt(2)/2*total))
        # print((blue/total)*((blue-1)/(total-1)).n(100))
    print("\t%d total, %d blue, %d red" %(total, blue, total-blue))
    total+=1
```

    	21 total, 15 blue, 6 red
    	120 total, 85 blue, 35 red
    	697 total, 493 blue, 204 red
    	4060 total, 2871 blue, 1189 red
    	23661 total, 16731 blue, 6930 red
    	137904 total, 97513 blue, 40391 red
    	803761 total, 568345 blue, 235416 red


__Remarks__:
Note that as $x, t \to \infty$, $\frac{x-1}{t-1} \to \frac{x}{t}$ and so $\frac x t \approx \frac{\sqrt{2}}2$ We could easily write this to test it.

Note that this question can be translated to finding the integer solution to the Diophantine equation
\begin{align}
b(b-1) &= 2a(a-1); \;\; a,b \in \mathbb{Z}
\end{align}

OEIS is a wonderful resource for number theory questions, so look there after generating the first few terms... The total is described in this list, where the 16th entry is the total number of chips in the solution. https://oeis.org/A046090/list. Parallel to that, the number of blue chips is described as this list https://oeis.org/A011900/list. Therefore, the solution is 756872327473
